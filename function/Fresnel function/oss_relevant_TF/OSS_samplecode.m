...
function [mask RfacR RfacF]= OSS_samplecode (F2D,supp,iter,beta,showim,modelimg,hiofirst) 
    ...
    for iteration = 1:iter
            ...
            if rem(iteration,2)==0
                ...
                if showim==1
                    ...
                    subplot(2,2,3), plot(RfacF), axis([0 ceil(iteration/2) 0 0.8]), title(int2str(errorF*100)); %µ¹Ò×¿Õ¼äÎó²î
                    subplot(2,2,4), plot(RfacR), axis([0 ceil(iteration/2) 0 0.8]), title(int2str(errorR*100)); %Êµ¿Õ¼äÎó²î
                    drawnow
                end
            end
    end
