...
function [mask,RfacR,RfacF,sample]= OSS_samplecode_TF(F2D,supp,iter,beta,showim,modelimg,hiofirst,L,lambda,z,k_space) 
...
%                     subplot(2,2,4), plot(RfacR), axis([0 ceil(iteration/2) 0 0.8]), title(int2str(errorR*100)); %实空间误差
                    ...
                    figure(63), plot(RfacF),xlabel('iterationNum');ylabel('rec errorF') , title(num2str(errorF*100)); %倒易空间误差%wy changed
                    figure(64), plot(RfacR),xlabel('iterationNum');ylabel('rec errorR') , title(num2str(errorR*100)); %实空间误差%wy changed
                    drawnow
                end
            end
    end
