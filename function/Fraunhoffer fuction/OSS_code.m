...
function [mask,RfacR,RfacF,sample]= OSS_code(F2D,supp,iter,beta,showim,modelimg,hiofirst) 
...
%                 subplot(2,2,4), plot(RfacR), axis([0 ceil(iteration/2) 0 0.8]), title(int2str(errorR*100)); 
                figure(61), 
                imagesc(abs(realgroup2(:,:,1))), axis square, colormap('gray');title(strcat(int2str(FX(iteration)),'--OSS')); %wy changed
