...
function [mask,RfacR,RfacF,sample]= OSS_lenscode(F2D,supp,iter,beta,showim,modelimg,hiofirst,k_space) 
...
%                 subplot(2,2,4), plot(RfacR), axis([0 ceil(iteration/2) 0 0.8]), title(int2str(errorR*100)); 
                figure(71), imagesc(abs(realgroup2(:,:,1))), axis square, colormap('gray');title(strcat(int2str(FX(iteration)),'--OSS lens')); %wy changed
                figure(72), imagesc(abs(realgroup2(:,:,2))), axis square, colormap('gray');title(['lens oss重建 iteration = ',int2str(iteration)]);%wy changed
