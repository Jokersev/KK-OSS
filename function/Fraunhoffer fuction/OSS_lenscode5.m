% OSS Source code: oss代码学习：插入GENFIRE_OSS的滤波部�?

function [mask,RfacR,RfacF,sample]= OSS_lenscode5(F2D,supp,iter,beta,showim,modelimg,hiofirst,k_space) 
...
%                 figure(72), imagesc(abs(realgroup2(:,:,2))), axis square, colormap('gray');title(['oss重建 iteration = ',int2str(iteration)]);%wy changed
                ...
                figure(74), plot(RfacR),xlabel('iterationNum');ylabel('lens errorR') , title(['errorR = ',mat2str(errorR,3)]); %实空间误�?%wy changed
