% OSS Source code: oss代码学习：插入GENFIRE_OSS的滤波部分，按迭代情况使用滤波器

function [mask,RfacR,RfacF,sample]= OSS_samplecode4(F2D,supp,iter,beta,showim,modelimg,hiofirst,k_space) 
 
%% General assignment of variables
[Rsize,Csize] = size(F2D);
R2D=zeros(Rsize,Csize,10,'single');  %过采样的实空间图�?
toperrs=single(1:10:100);
kfilter=zeros(Rsize,Csize,'single');   %滤波�?
realgroup=zeros(Rsize,Csize,2,'single');
realgroup(:,:,1)=modelimg;       %realgroup第一个放置的是模�?
 
%% Assign variables
stopper = find(F2D==-1);
filtercount=10;     %有十种滤波器，α取了十个�?�对应的滤波�?
...
%% Compute filter parameter alpha  计算滤波器参数�?
X=1:iter; %x=1�?2�?3�?...，n次迭�?
FX=(filtercount+1-ceil(X*filtercount/iter))*ceil(iter/(1*filtercount)); 
FX=((FX-ceil(iter/filtercount))*(2*Rsize)/max(FX))+(2*Rsize/10);   %滤波器尺�?
figure(98), plot(X,FX), axis tight; title('OSS Filter Size v. Iterations');
 
%% Generate initial filter 产生初始滤波�?
for kk=1:Rsize
    for jj=1:Csize
        kfilter(kk,jj)=exp( -( ( (sqrt((kk-Rcenter)^2+(jj-Ccenter)^2).^2) ) ./ (2* FX(1)^2) ));
    end
end
kfilter=kfilter/max(max(kfilter)); %滤波�?
 
%% Assign random phases 随机相位
rng('shuffle','twister'); %产生随机�?
% phase_angle = rand(Rsize,Csize,'single'); %产生合�?�大小的随机相位
 
%% Define initial k, r space 定义初始衍射值和初始物体
% initial_k=F2D; initial_k(initial_k==-1)=0;     
% k_space = initial_k.*exp(1i*phase_angle);      
% buffer_r_space = single(real(ifftn(ifftshift(k_space)))); %缓冲实空间图像，single单精度类型减少内存，real取实�?
...
%         sample(sample<0)=r_space(sample<0);       %正则约束，先保留
        ...
        %% Apply frequency filter (OSS) 应用频率滤波�?
        if hiofirst==0 || iteration>ceil(iter/filtercount)
            ...
            kfilter=kfilter/max(max(kfilter)); %按照迭代次数的情况�?�择相应的滤波器
%             ktemp=fftshift(fftn(r_space));   %支持域外的部分的傅里叶变�?
            ktemp=fft2(r_space);
...
%             r_space=single(real(ifftn(ifftshift(ktemp))));    %滤波之后支持域外的部分返回实空间
        ...
        %% Use best result from last filter 从最后的滤波器中选择支持域外部的�?佳结�?
        %wy changed
%         if mod(iteration,ceil(iter/filtercount))==0   %mod求取向量被除后的余数，ceil向上取整;求此时的 迭代次数�?(总迭�?/10) 除后的余�?
%             r_space=R2D(:,:,filtnum);  
%         else
...
%         k_space = fftshift(fftn(r_space));  
        ...
        k_space = F2D.*exp(1i*phase_angle);   %保留新相位，形成新衍射图�?
...
%             Ktemp = abs(fftshift(fftn(Ktemp)));
            ...
            %% Determine interations with best error 确定具有�?佳误差的交互作用
            %wy changed
...
%                 figure(62), imagesc(abs(realgroup2(:,:,2))), axis square, colormap('gray');title(['oss重建 iteration = ',int2str(iteration)]);%wy changed
                ...
                figure(64), plot(RfacR),xlabel('iterationNum');ylabel('rec errorR') , title(['errorR = ',mat2str(errorR,3)]); %实空间误�?%wy changed
