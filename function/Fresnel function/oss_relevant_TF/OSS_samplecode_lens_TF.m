...
function [mask,RfacR,RfacF,sample]= OSS_samplecode_lens_TF(F2D,supp,iter,beta,showim,modelimg,hiofirst,L,lambda,z,k_space) 
...
%     RfacF = zeros(iter,1,'single');  counter1=0; errorF=1;     %wy chagned
%     RfacR = zeros(iter,1,'single');  counter2=0; errorR=1;     %wy chagned
...
%                     subplot(2,2,4), plot(RfacR), axis([0 ceil(iteration/2) 0 0.8]), title(int2str(errorR*100)); %实空间误差
                    ...
                    figure(73), plot(RfacF),xlabel('iterationNum');ylabel('lens errorF') , title(['errorF = ',int2str(errorF*100)]); %倒易空间误差%wy changed
                    figure(74), plot(RfacR),xlabel('iterationNum');ylabel('lens errorR') , title(['errorR = ',int2str(errorR*100)]); %实空间误差%wy changed
                    drawnow
                end
            end
    end
