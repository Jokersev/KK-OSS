...
%                 figure(72), imagesc(abs(realgroup2(:,:,2))), axis square, colormap('gray');title(['oss重建 iteration = ',int2str(iteration)]);%wy changed
                figure(73), plot(RfacF),xlabel('iterationNum/2');ylabel('rec errorF') , title(['errorF = ',int2str(errorF*100),'%']); %倒易空间误差%wy changed
                figure(74), plot(RfacR),xlabel('iterationNum');ylabel('rec errorR') , title(['errorR = ',mat2str(errorR,3)]); %实空间误�?%wy changed
