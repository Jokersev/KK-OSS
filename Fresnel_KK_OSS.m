...
supportlens_pad = padarray(u1,[paddingx paddingy]);  %透镜的支持域
...
% figure  
% subplot(1,2,1)
% imagesc(x1,y1,abs(initial_guess_field));axis square;colormap('gray');
% title('initial guess object','fontsize',22)
% subplot(1,2,2)
% imagesc(x1,y1,abs(initial_guess_lens_field));axis square;colormap('gray');
% title('initial guess lens','fontsize',22)
%% ER迭代重建
rec_modelimg = kcdi_real; %过采样后的物体模型
lens_modelimg = kcdi_lens_real; %过采样后样品面上的透镜模型
Iterations = 1;
[rec,rec_lens] = ER_reconstruction2(x1,y1,Iterations,initial_field,initial_diffraction,initial_lens_field,initial_lens_diffraction,support_pad,supportlens_pad,paddingx,paddingy,L1,lambda,z2,z,am_p,am_p_lens,vecX,ncX_big,vecY,ncY_big,ph_p_orig,ph_p_orig_lens,lens_modelimg,rec_modelimg);
%% OSS重建，支持域为200*200的正方形，rec（2048*2048）为样品过采样后的结果
%% er重建结果在实空间进行过采样，进行衍射图相加
rec_lens = rec_lens.*support;
sample_diffraction = propTF(rec,L1,lambda,z2); 
lens_diffraction = propTF(rec_lens,L1,lambda,z2); 
rec_k_space =  sample_diffraction + lens_diffraction;
...
hiofirst = 0;
%%
[mask_sample,RfacR_sample,RfacF_sample,rec]= OSS_samplecode_TF2(F2D_rec,supp,iter,beta,showim,rec_modelimg,hiofirst,L1,lambda,z2,rec_k_space);  
[mask_lens,RfacR_lens,RfacF_lens,rec_lens]= OSS_samplecode_lens_TF2(F2D_lens,supp,iter,beta,showim,lens_modelimg,hiofirst,L1,lambda,z2,lens_diffraction);  
%重建时间
reconstructionTime = toc;
reconstructionTime = round(10*reconstructionTime)./10;
fprintf('KK-OSS: Reconstruction completed in %.12g seconds.\n\n',reconstructionTime);
